 ...
 */
export function createErrorView(message: string, title = 'Lock Code Manager'): LovelaceViewConfig {
    return {
        cards: [
            {
                content: message,
...
export function formatConfigEntryNotFoundError(
    ...
    return `## ERROR: No Lock Code Manager configuration ${content} found!`;
}
