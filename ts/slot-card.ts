...
class LockCodeManagerSlotCard extends LcmSlotCardBase {
    ...
    private _renderFromData(data: SlotCardData): TemplateResult {
        ...
        const hasConditions =
            conditions.condition_entity !== undefined || conditions.calendar !== undefined;
    ...
    private _renderConditionsSection(conditions: SlotCardConditions): TemplateResult {
        ...
        const hasConditions = hasConditionEntity;
        const totalConditions = hasConditionEntity ? 1 : 0;
        const blockingConditions = entityBlocking ? 1 : 0;

        // Show checkmark when all conditions pass (muted), X when blocking (prominent)
    ...
    private async _updateSlotCondition(updates: { entity_id?: string | null }): Promise<void> {
        if (!this._hass || !this._config) {
