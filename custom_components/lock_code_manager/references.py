...
def _rewrite(value: str, moved: Mapping[str, str]) -> str:
    ...
    Assistant reports as a reference.

    ONE pass over the string, not one pass per rename. Applying the renames
    in turn lets a value rewritten by an earlier rule be rewritten again by a
    later one, so ``a -> b`` and ``b -> c`` in the same batch would carry an
    ``a`` all the way to ``c``.

    The boundaries treat ``_`` and ``.`` as parts of an ID, so a longer ID
    that merely begins with a moved one is left alone.
    """
    if not moved:
        return value
    pattern = "|".join(re.escape(old) for old in sorted(moved, key=len, reverse=True))
    return re.sub(
        rf"(?<![\w.])({pattern})(?![\w.])", lambda match: moved[match.group(1)], value
    )
...
def _repoint_file(
    ...
    try:
        ...
        # The user asked for these to be updated, so failing quietly and
        # reporting a count would be a lie about what happened.
        _LOGGER.exception("Could not write %s", _FILES[domain])
        raise
    return changed
