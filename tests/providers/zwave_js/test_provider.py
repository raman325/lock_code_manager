...
async def test_set_credential_confirmed_pushes_the_value_like_every_push_provider(
    ...
    Every other push provider pushes what it just wrote before returning
    CONFIRMED, and the seam records nothing pending for a push provider on
    exactly that strength. Without the push the coordinator kept the
    pre-write state -- an empty slot, verified -- and on a mesh lossy enough
    for the reconcile read to time out, no driver event followed; the sync
    tick then took a verified empty slot as the lock's answer and rewrote,
    three times over, a code the lock had held all along.
    """
