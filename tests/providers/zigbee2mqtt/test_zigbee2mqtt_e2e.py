...
These tests go through the full LCM config entry setup lifecycle: LCM discovers
the MQTT lock entity, instantiates Zigbee2MQTTLock, creates the coordinator,
entities, and sync managers. MQTT subscribe/publish are intercepted at the HA
component boundary via MqttMessageBus, so the provider code runs unpatched.
"""
